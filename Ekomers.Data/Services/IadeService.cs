...
namespace Ekomers.Data.Services
{
	...
	public class IadeService : IIadeService
	{
		...
		private readonly IRepository<MalzemeIade> _iadeRepo;
		...
		public IadeService(IMapper mapper, ApplicationDbContext context,
			...
			IRepository<MalzemeGrup> altGrupRepo, IRepository<MalzemeBirim> malzemeBirimRepo, IRepository<MalzemeIade> stokRepo, IRepository<MalzemeHareketTur> hareketTurRepo
		...
		public IQueryable<MalzemeIadeVM> GenelListe()
		{
			var result = from stok in _iadeRepo.GetAll2(a => a.IsActive == true && a.IsDelete == false)
		...
		public Task<MalzemeIadeVM> VeriDoldurGenel(params string[] listTypes)
		{
			throw new NotImplementedException();
		}
		public bool VeriEkle(MalzemeIadeVM modelv)
		{
			MalzemeHareketTur tur = _hareketTurRepo.GetById(modelv.HareketTurID);
			MalzemeIade hareket = _iadeRepo.GetById(modelv.ID);
			if (hareket != null)
			{
				...
				_iadeRepo.Update(hareket);
			}
			else
			{
				var model = new MalzemeIade
				{
		...
		public async Task<bool> VeriEkleCoklu(MalzemeIadeVM modelv)
		{
			var newGuid = Guid.NewGuid().ToString();
			foreach (MalzemeIadeVM item in modelv.MalzemeIadeVMListe)
			{
				var model = new MalzemeIade
				{
		...
		public async Task<bool> VeriEkleTransferCoklu(MalzemeIadeVM modelv)
		{
			var newGuid = Guid.NewGuid().ToString();
			foreach (MalzemeIadeVM item in modelv.MalzemeIadeVMListe)
			{
				...
				var modelTransfer = new MalzemeIade
				{
					MalzemeID = item.MalzemeID,
					Miktar = item.Miktar,
					Aciklama = modelv.Aciklama,
		...
		public async Task<MalzemeIadeVM> VeriGetir(int HareketID)
		{
			if (HareketID <= 0)
			{
				return new MalzemeIadeVM();
			}

			MalzemeIadeVM hareket = GenelListe().Where(p => p.ID == HareketID).FirstOrDefault();
			if (hareket == null)
			{
				return new MalzemeIadeVM();
			}
		...
		public Task<List<MalzemeIadeVM>> VeriListele()
		{
			throw new NotImplementedException();
		}
		public async Task<List<MalzemeIadeVM>> VeriListele(MalzemeIadeVM model, int departmanID)
		{
		...
		public async Task<List<MalzemeIadeVM>> VeriListele(int departmanID)
		{
		...
		public Task<List<MalzemeIadeVM>> VeriListele(MalzemeIadeVM model)
		{
			throw new NotImplementedException();
		}
		...
		{
			MalzemeIade stok = _iadeRepo.GetById(HareketID);
		...
		public async Task<bool> MalzemeEkle(MalzemelerVM modelv)
		{
			...
			else
			{
				...
				malzeme.GrupID = (int)modelv.AltGrupID;
		...
		public async Task<List<MalzemelerVM>> DepoDurumu(int departmanID)
		{
			...
			var stokHareketListesi = _iadeRepo.GetAll2(a => a.IsActive == true && a.IsDelete == false);
