...
public sealed class UretimYonetimiController : Controller
{
    ...
    public async Task<IActionResult> DepoGoreviOlustur(int id,CancellationToken ct)
    {
        ...
        if(task!=null)
        {
            ...
            await _context.SaveChangesAsync(ct);
            foreach(var allocation in sourceAllocations){var item=itemByRequirement[allocation.RequirementId];_context.PrdWarehouseTaskLots.Add(new PrdWarehouseTaskLot{WarehouseTaskItemId=item.ID,StockReservationId=allocation.Reservation.ID,StockLotId=allocation.Reservation.StockLotId,Quantity=allocation.Reservation.ReservedQuantity,IsActive=true,IsDelete=false,CreateDate=now,CreateUserID=user});}
    ...
    {
        ViewBag.Modul="YeniUretim";var model=await(from task in _context.PrdWarehouseTasks.AsNoTracking() join order in _context.PrdProductionOrders.AsNoTracking() on task.ProductionOrderId equals order.ID join product in _context.PrdMaterials.AsNoTracking() on order.ProductMaterialId equals product.ID join source in _context.PrdWarehouses.AsNoTracking() on task.SourceWarehouseId equals source.ID join target in _context.PrdWarehouses.AsNoTracking() on task.TargetWarehouseId equals target.ID where task.ID==id&&task.IsDelete!=true select new ProductionWarehouseTaskDetailVM{Id=task.ID,TaskNumber=task.TaskNumber,ProductionOrderId=order.ID,OrderNumber=order.OrderNumber,ProductCode=product.Code,ProductName=product.Name,SourceWarehouse=source.Code+" - "+source.Name,TargetWarehouse=target.Code+" - "+target.Name,Status=task.Status,RequestDate=task.RequestDate,AssignedUserId=task.AssignedUserId,PreparedDate=task.PreparedDate,ShippedDate=task.ShippedDate,DeliveredDate=task.DeliveredDate,Notes=task.Notes}).FirstOrDefaultAsync(ct);if(model==null)return NotFound();
        model.Items=await(from item in _context.PrdWarehouseTaskItems.AsNoTracking() join material in _context.PrdMaterials.AsNoTracking() on item.MaterialId equals material.ID join unit in _context.PrdUnits.AsNoTracking() on item.UnitId equals unit.ID where item.WarehouseTaskId==id&&item.IsDelete!=true orderby material.Code select new ProductionWarehouseTaskItemVM{Id=item.ID,MaterialCode=material.Code,MaterialName=material.Name,Unit=unit.Name,RequestedQuantity=item.RequestedQuantity,PreparedQuantity=item.PreparedQuantity,ShippedQuantity=item.ShippedQuantity,ShortageQuantity=item.ShortageQuantity}).ToListAsync(ct);var itemIds=model.Items.Select(x=>x.Id).ToList();var lots=await(from taskLot in _context.PrdWarehouseTaskLots.AsNoTracking() join stockLot in _context.PrdStockLots.AsNoTracking() on taskLot.StockLotId equals stockLot.ID join material in _context.PrdMaterials.AsNoTracking() on stockLot.MaterialId equals material.ID join unit in _context.PrdUnits.AsNoTracking() on material.UnitId equals unit.ID where itemIds.Contains(taskLot.WarehouseTaskItemId)&&taskLot.IsDelete!=true select new{taskLot.WarehouseTaskItemId,stockLot.LotNumber,stockLot.ExpirationDate,taskLot.Quantity,Unit=unit.Name}).ToListAsync(ct);foreach(var item in model.Items)item.Lots=lots.Where(x=>x.WarehouseTaskItemId==item.Id).Select(x=>new ProductionWarehouseTaskLotVM{LotNumber=x.LotNumber,ExpirationDate=x.ExpirationDate,Quantity=x.Quantity,Unit=x.Unit}).ToList();model.ItemCount=model.Items.Count;model.RequestedQuantity=model.Items.Sum(x=>x.RequestedQuantity);model.PreparedQuantity=model.Items.Sum(x=>x.PreparedQuantity);model.ShortageQuantity=model.Items.Sum(x=>x.ShortageQuantity);return View(model);
    }
    ...
    [HttpPost,ValidateAntiForgeryToken]
    public async Task<IActionResult> DepoGoreviHazirlandi(int id,CancellationToken ct)
    {
        var task=await _context.PrdWarehouseTasks.FirstOrDefaultAsync(x=>x.ID==id&&x.IsDelete!=true,ct);if(task==null)return NotFound();if(task.Status!=PrdWarehouseTaskStatus.Preparing){TempData["error"]="Önce depo görevini hazırlamaya başlatınız.";return RedirectToAction(nameof(DepoGorevDetay),new{id});}var items=await _context.PrdWarehouseTaskItems.Where(x=>x.WarehouseTaskId==id&&x.IsDelete!=true).ToListAsync(ct);foreach(var item in items)item.PreparedQuantity=item.RequestedQuantity;var hasShortage=items.Any(x=>x.ShortageQuantity>0);task.Status=hasShortage?PrdWarehouseTaskStatus.Shortage:PrdWarehouseTaskStatus.Ready;task.PreparedDate=DateTime.Now;task.UpdateDate=DateTime.Now;task.UpdateUserID=User.Identity?.Name;var order=await _context.PrdProductionOrders.FirstAsync(x=>x.ID==task.ProductionOrderId,ct);order.Status=hasShortage?PrdProductionOrderStatus.MaterialWaiting:PrdProductionOrderStatus.WarehousePreparing;order.UpdateDate=DateTime.Now;order.UpdateUserID=User.Identity?.Name;await _context.SaveChangesAsync(ct);TempData[hasShortage?"error":"success"]=hasShortage?"Ayrılan malzemeler hazırlandı; eksik miktarlar görevde açık kalmaya devam ediyor.":"Ayrılan malzemeler hazırlandı; sevk aşamasına geçilebilir.";return RedirectToAction(nameof(DepoGorevDetay),new{id});
    }
    ...
    public async Task<IActionResult> DepoGoreviSevkEt(int id,CancellationToken ct)
    {
        ...
        var taskLots=await(from taskLot in _context.PrdWarehouseTaskLots
                           ...
                           where itemIds.Contains(taskLot.WarehouseTaskItemId)&&taskLot.IsDelete!=true&&reservation.IsDelete!=true&&reservation.WarehouseId==task.SourceWarehouseId&&(reservation.Status==PrdReservationStatus.Active||reservation.Status==PrdReservationStatus.PartiallyUsed)
        ...
        try
        {
            ...
            {
                var sourceLot=allocation.SourceLot;var reservation=allocation.Reservation;var quantity=allocation.TaskLot.Quantity;var balance=balanceRows[sourceLot.ID];var unitCost=balance.Quantity==0?0:balance.Value/balance.Quantity;var totalCost=quantity*unitCost;var targetLot=await GetOrCreateProductionTargetLotAsync(sourceLot,task.TargetWarehouseId,now,user,ct);
    ...
    public async Task<IActionResult> GerceklesenTamamla(ProductionExecutionDetailVM input,CancellationToken ct)
    {
        ...
        if(returnDocument!=null)returnDocument.TotalCost=returnTotal;var receiptDocument=new PrdInventoryDocument{DocumentNumber=$"UMAM-{now:yyyyMMddHHmmssfff}-{order.ID}",Type=PrdInventoryDocumentType.ProductionReceipt,Status=PrdInventoryDocumentStatus.Posted,DocumentDate=input.ProductionDate,PostingDate=now,PostedUserId=user,TargetWarehouseId=order.ProductionWarehouseId,CurrencyCode="TRY",ExchangeRate=1,TotalCost=netMaterialCost,SourceDocumentType="ProductionOrder",SourceDocumentId=order.ID,Notes=$"{order.OrderNumber} mamul girişi",IsActive=true,IsDelete=false,CreateDate=now,CreateUserID=user};_context.PrdInventoryDocuments.Add(receiptDocument);await _context.SaveChangesAsync(ct);var resultLot=await GetOrCreateProductionLotAsync(order.ProductMaterialId,order.ProductionWarehouseId,order.BatchNumber,input.ProductionDate,input.ExpirationDate,now,user,ct);var outputUnitCost=netMaterialCost/outputQuantity;var receiptLine=new PrdInventoryDocumentLine{InventoryDocumentId=receiptDocument.ID,Sequence=1,MaterialId=order.ProductMaterialId,UnitId=order.UnitId,TargetStockLotId=resultLot.ID,LotNumber=resultLot.LotNumber,ProductionDate=input.ProductionDate,ExpirationDate=input.ExpirationDate,Quantity=outputQuantity,OriginalUnitCost=outputUnitCost,CurrencyCode="TRY",ExchangeRate=1,UnitCost=outputUnitCost,TotalCost=netMaterialCost,CostSource=PrdStockCostSource.Production,Notes=input.Notes?.Trim(),IsActive=true,IsDelete=false,CreateDate=now,CreateUserID=user};_context.PrdInventoryDocumentLines.Add(receiptLine);await _context.SaveChangesAsync(ct);_context.PrdStockMovements.Add(CreateProductionStockMovement(receiptDocument,receiptLine,order.ProductionWarehouseId,resultLot.ID,PrdStockDirection.In,PrdStockMovementType.ProductionReceipt,order.ID,now,user));_context.PrdProductionResults.Add(new PrdProductionResult{ProductionOrderId=order.ID,ProductMaterialId=order.ProductMaterialId,WarehouseId=order.ProductionWarehouseId,UnitId=order.UnitId,ActualQuantity=outputQuantity,BatchNumber=order.BatchNumber,ProductionDate=input.ProductionDate,ExpirationDate=input.ExpirationDate,StockLotId=resultLot.ID,Notes=input.Notes?.Trim(),IsActive=true,IsDelete=false,CreateDate=now,CreateUserID=user});order.ActualQuantity=outputQuantity;order.Status=PrdProductionOrderStatus.Completed;order.CompletionDate=now;order.UpdateDate=now;order.UpdateUserID=user;await _context.SaveChangesAsync(ct);await transaction.CommitAsync(ct);TempData["success"]="Üretim tamamlandı; tüketim, iade, fire ve mamul stok kayıtları işlendi.";return RedirectToAction(nameof(GerceklesenDetay),new{id=order.ID});
    }
    ...
    private async Task<ProductionExecutionDetailVM?> BuildProductionExecutionDetailAsync(int orderId,ProductionExecutionDetailVM? posted,CancellationToken ct)
    {
        ...
        var result=await _context.PrdProductionResults.AsNoTracking().Where(x=>x.ProductionOrderId==orderId&&x.IsDelete!=true).OrderByDescending(x=>x.ID).FirstOrDefaultAsync(ct);if(result!=null){model.ProductionDate=result.ProductionDate;model.ExpirationDate=result.ExpirationDate;model.ActualQuantityInput=result.ActualQuantity.ToString("0.######",CultureInfo.InvariantCulture);model.Notes=result.Notes;}
