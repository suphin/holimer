(() => {
    ...
    set_letter_head: function(frm) {
      if (frm.fields_dict.letter_head) {
        frappe.db.get_value("Company", frm.doc.company, "default_letter_head").then((res) => {
          var _a;
          if ((_a = res.message) == null ? void 0 : _a.default_letter_head) {
        ...
        batch_fields = [
          {
            ...
            get_query: () => {
              let is_inward = false;
              if (["Purchase Receipt", "Purchase Invoice"].includes(this.frm.doc.doctype) && !this.frm.doc.is_return || this.frm.doc.doctype === "Stock Entry" && (this.frm.doc.purpose === "Material Receipt" || this.frm.doc.purpose === "Manufacture" && this.item.is_finished_item)) {
                is_inward = true;
              }
  ...
  // frappe-html:/home/frappe/frappe-bench/apps/erpnext/erpnext/public/js/templates/visual_plant_floor_template.html
...
`;
  ...
  erpnext.taxes_and_totals = class TaxesAndTotals extends erpnext.payments {
    ...
    async calculate_taxes_and_totals(update_paid_amount) {
      ...
      if (this.frm.doc.doctype === "Purchase Invoice" && this.frm.doc.is_return && this.frm.doc.grand_total < 0 && this.frm.doc.grand_total > this.frm.doc.paid_amount) {
    ...
    calculate_totals() {
      ...
      if (["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "POS Invoice"].includes(
        this.frm.doc.doctype
      )) {
    ...
    set_default_payment(total_amount_to_pay, update_paid_amount) {
      ...
      if (this.frm.doc.is_pos && !cint(this.frm.skip_default_payment) && (update_paid_amount === void 0 || update_paid_amount)) {
        $.each(this.frm.doc["payments"] || [], function(index, data) {
  ...
  erpnext.TransactionController = class TransactionController extends erpnext.taxes_and_totals {
    ...
    company() {
      ...
      var set_party_account = function(set_pricing2) {
        ...
        if (["Sales Invoice", "Purchase Invoice"].includes(me.frm.doc.doctype)) {
          ...
          if (party && me.frm.doc.company && (!((_a = me.frm.doc.__onload) == null ? void 0 : _a.load_after_mapping) || !me.frm.doc[party_account_field])) {
            return frappe.call({
      ...
      if (frappe.meta.get_docfield(this.frm.doctype, "shipping_address") && ["Purchase Order", "Purchase Receipt", "Purchase Invoice"].includes(this.frm.doctype) && !this.frm.doc.shipping_address) {
    ...
    plc_conversion_rate() {
      ...
      } else if (this.frm.doc.price_list_currency === this.frm.doc.currency && this.frm.doc.plc_conversion_rate && flt(this.frm.doc.plc_conversion_rate) != 1 && flt(this.frm.doc.plc_conversion_rate) != flt(this.frm.doc.conversion_rate)) {
        ...
        return this.frm.call({
          ...
          callback: function(r) {
            if (!r.exc) {
              ...
              taxes.forEach((tax) => {
                var _a;
                if (((_a = me.frm.doc) == null ? void 0 : _a.cost_center) && !tax.cost_center) {
    ...
    make_quality_inspection() {
      ...
      const dialog = new frappe.ui.Dialog({
        ...
        primary_action: function() {
          ...
          const selected_data = data2.items.filter((item) => (item == null ? void 0 : item.__checked) == 1);
          frappe.call({
  ...
  erpnext.utils.BarcodeScanner = class BarcodeScanner {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.scan_api_call(input, (r) => {
          ...
          if (!data || Object.keys(data).length === 0 || data.warehouse && !this.has_last_scanned_warehouse) {
            this.show_alert(
              this.has_last_scanned_warehouse ? __("Cannot find Item or Warehouse with this Barcode") : __("Cannot find Item with this Barcode"),
              "red"
            );
            this.clean_up();
    ...
    update_table(data) {
      return new Promise((resolve, reject) => {
        ...
        const is_new_row = !(row == null ? void 0 : row.item_code);
        if (!row) {
    ...
    async set_warehouse(row) {
      ...
      if (!warehouse_field || !frappe.meta.has_field(row.doctype, warehouse_field))
        return;
    ...
    get_row_to_modify_on_scan(item_code, batch_no, uom, barcode, default_warehouse) {
      ...
      const matching_row = (row) => {
        ...
        return item_match && uom_match && warehouse_match && !item_scanned && (!is_batch_no_scan || batch_match) && (!check_max_qty || qty_in_limit);
      };
    ...
    setup_selling_controller: function() {
      erpnext.selling.SellingController = class SellingController extends erpnext.TransactionController {
        ...
        set_default_company_contact_person() {
          ...
          if (this.frm.doc.company) {
            frappe.db.get_value("Company", this.frm.doc.company, "default_sales_contact").then((r) => {
              var _a;
              if ((_a = r.message) == null ? void 0 : _a.default_sales_contact) {
    ...
    open_general_ledger: function(data) {
      ...
      filters.forEach((f) => {
        if (f.df.fieldtype == "MultiSelectList") {
          if (f.df.fieldname in frappe.route_options)
            return;
...
//# sourceMappingURL=erpnext.bundle.SU4D3T7M.js.map
